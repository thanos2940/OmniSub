"""
Cache Manager — Gemini Context Caching for OmbiSub
...
def invalidate_cache(metadata: Dict) -> Dict:
    ...
    if old_cache_name:
        try:
            ...
            print(f"DEBUG: Deleted old cache: {old_cache_name}")
        except Exception as e:
            # Cache may have already expired or been deleted — that's fine
            print(f"DEBUG: Could not delete cache '{old_cache_name}': {e}")
...
def get_or_create_cache(
    ...
    if stored_name and stored_fingerprint == current_fingerprint:
        ...
        if stored_expiry:
            try:
                ...
                if expiry_dt > datetime.now(timezone.utc):
                    print(f"DEBUG: Reusing valid cache: {stored_name}")
        ...
        try:
            ...
            print(f"DEBUG: Reusing valid cache (no expiry check): {stored_name}")
            return stored_name, metadata
        except Exception:
            print(f"DEBUG: Stored cache '{stored_name}' no longer exists")
    ...
    try:
        ...
        cache = client.caches.create(
            ...
            config=genai_types.CreateCachedContentConfig(
                display_name=f"ombisub_{metadata.get('show_name', 'project')}",
        ...
        print(f"DEBUG: Created new cache: {cache.name} (fingerprint: {current_fingerprint})")
        return cache.name, metadata

    except Exception as e:
        print(f"DEBUG: Failed to create cache (will proceed without): {e}")
        metadata["context_cache_name"] = None
        metadata["context_cache_fingerprint"] = current_fingerprint
        metadata["context_cache_expiry"] = None
