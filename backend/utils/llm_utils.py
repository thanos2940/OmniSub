...
# Outermost JSON object or array, across newlines.
_JSON_SPAN_RE = re.compile(r'(\{.*\})|(\[.*\])', re.DOTALL)


def extract_json_text(text: str) -> str:
    """Return the most likely JSON substring from a model response.

    Strips ```json / ``` fences and isolates the outermost {...} or [...] span.
    Shared by the structured-output parser and the inline JSON consumers so the
    fence/brace handling lives in exactly one place.
    """
    if not text:
        return ""
    ...
    m = _JSON_SPAN_RE.search(candidate)
    if m:
        candidate = m.group(0)
    return candidate


def loads_tolerant(text: str) -> Optional[Any]:
    """Best-effort JSON parse: isolate the JSON span, drop trailing commas, parse.

    Returns the decoded object, or None if nothing parseable was found.
    """
    candidate = extract_json_text(text)
    if not candidate:
        return None
    candidate = re.sub(r',\s*([\]}])', r'\1', candidate)  # tolerate trailing commas
    try:
...
def parse_glossary_from_text(text: str) -> dict:
    ...
    if "<glossary>" in clean_text or "<term>" in clean_text:
        ...
        for block in term_blocks:
            term_data = {}
            # Standard tags
            for tag in ["source", "term", "translation", "description", "context_quote", "type", "gender", "case_sensitive", "keep_original"]:
...
def parse_translations_from_text(text: str, expected_count: int = 0) -> List[Dict]:
    ...
    if pipe_matches:
        # Build ordered list preserving continuation lines (lines without N| prefix)
        ...
        for line in clean_text.split('\n'):
            ...
            if pipe_m:
                ...
                idx_map[current_idx] = pipe_m.group(2).strip()
            elif current_idx is not None and current_idx in idx_map:
                # Continuation line — append to the current entry
                idx_map[current_idx] += " " + stripped

        for k in idx_map:
            idx_map[k] = idx_map[k].replace("<br>", "\n")
