...
async def run_review(
    ...
    from utils.rate_limiter import active_model_var
...
def apply_review_results(
    ...
    for result in results:
        ...
        if result.needs_retranslation and result.suggestion:
            old_trans = parsed_srt[idx].get("translated") or ""
            ...
            if lang_code:
                parsed_srt[idx]["translations"][lang_code] = result.suggestion
            ...
            if project and episode:
                try:
                    ...
                    record_feedback(
                        ...
                        episode=episode,
                        line_index=idx,
                        source=parsed_srt[idx].get("original", ""),
                        bad_target=old_trans,
                        corrected_target=result.suggestion,
                        reason=f"LLM Reviewer correction: {result.issues}",
                        lang_code=lang_code
                    )
