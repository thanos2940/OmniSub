"""
ADK Agent Operations - High-level Functions for Omnisub
...
STEP 1: Extract every named entity and special term from the SUBTITLE TEXT below.
STEP 2: Using your knowledge of "{show_name}", add ALL important characters, locations, organizations, items, and concepts from the ENTIRE show/movie — even those not mentioned in this text sample. The sample is only a small slice; the glossary must cover the full series.

Aim for 40-80+ terms total. Be comprehensive — translators need every proper noun and special term.
...
{text_sample}"""
    ...
    try:
        ...
        logger.warning(f"generate_glossary_adk failed: {e}")
...
## Tone & Style"""
    ...
    try:
        ...
        logger.warning(f"research_project_adk failed: {e}")
...
{"Additional Context: " + context_guide if context_guide else ""}"""
    ...
    try:
        ...
        logger.warning(f"translate_batch_adk failed: {e}")
...
async def enhance_context_guide_adk(
    ...
    prompt = f"""Write a subtitle translation style guide for "{show_name}" translating into {target_language}.
Max 200 words. Use EXACTLY this structure — no extra sections:

TONE: [one sentence describing overall mood/register tailored for {target_language}]
FORMALITY: [formal/informal/mixed — one sentence explaining how to handle this in {target_language}]
REGISTER: [2-3 bullets for character-specific speech patterns, if relevant]
CULTURAL: [2-3 bullets for adaptation rules — idioms, honorifics, etc. when translating to {target_language}]
SPECIAL: [any unique handling rules for this show's specific terminology in {target_language}]
...
{research_data}"""
    ...
    try:
        ...
        if not response_text:
            logger.debug(f"Context Agent returned empty response for '{show_name}'")
        ...
        logger.warning(f"enhance_context_guide_adk failed: {e}")
...
def _parse_numbered_output(text: str, expected_count: int) -> List[str]:
    ...
    # Try pipe-delimited first (primary format) — with continuation-line support
    ...
    if pipe_matches:
        ...
        for line in text.split('\n'):
            ...
            if not stripped:
                continue
            pipe_m = re.match(r'^(\d+)\s*\|\s*(.*)', stripped)
            if pipe_m:
                current_idx = int(pipe_m.group(1))
                if 1 <= current_idx <= expected_count:
                    lines_map[current_idx] = pipe_m.group(2).strip()
            elif current_idx is not None and current_idx in lines_map:
                # Continuation line — append to current entry
                lines_map[current_idx] += " " + stripped
    else:
        for line in text.split("\n"):
            stripped = line.strip()
            if not stripped:
...
async def generate_episode_summary_adk(
    ...
    try:
        summary = await generate(
            model_name=model_name,
            prompt=prompt,
            system_instruction="Write extremely concise episode summaries for subtitle translation context. Output ONLY the summary text — no labels, no preamble, no markdown.",
            temperature=0.2,
            role="summary",
        )
        return summary.strip()
    except Exception as e:
        logger.warning(f"generate_episode_summary_adk failed: {e}")
        return ""
